...
export async function checkAndIncrement(keyId, timezone = 'UTC') {
    let usage = await getKeyUsage(keyId);
    if (!usage)
        return { allowed: true, dailyCount: 0, dailyLimit: 2 };
    const now = new Date();
...
export async function getUsageStatus(keyId) {
    const usage = await getKeyUsage(keyId);
    if (!usage)
        return { dailyCount: 0, dailyLimit: 2, totalRequests: 0 };
    return {
        dailyCount: usage.daily_count,
        dailyLimit: usage.daily_limit,

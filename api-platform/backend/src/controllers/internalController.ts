...
export async function validateKey(req: Request, res: Response) {
  ...
  const { apiKey, tool, timezone } = req.body as { apiKey?: string; tool?: string; timezone?: string };

  if (!apiKey) {
    return res.status(400).json({ valid: false, error: 'api_key_required' });
  }

  const keyRecord = await validateApiKey(apiKey) as
    | { id: string; user_id: string; status: string; is_free: boolean }
    | null;

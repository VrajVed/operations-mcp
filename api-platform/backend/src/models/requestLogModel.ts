...
export async function logRequest(keyId: string, endpoint: string, method: string, statusCode: number, latencyMs: number) {
...
export async function getRequestLogsByUser(userId: string, limit = 50) {
  return db.select({
    id: requestLogs.id,
